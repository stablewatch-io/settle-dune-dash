-- IB Vaults Rewards Calculation (Morpho) — V1 (dynamic market discovery)
-- Query ID: 6925830
-- Dune: https://dune.com/queries/6925830
-- Description: Time-weighted USDS reward for Morpho V1 IB vaults (vault level).
--              Total idle USDS = vault cash balance + vault's pro-rata share of
--              unborrowed USDS across all Morpho Blue markets it supplies to.
--              Markets are discovered dynamically from historical supply events.
--
-- V1 vs V2 architecture:
--   V2: vault → adapter contract → Morpho Blue markets  (adapter is onBehalf)
--   V1: vault → Morpho Blue markets directly            (vault itself is onBehalf)
--   Therefore this query has no adapter address; all "adapter" logic from the V2
--   query uses the vault address directly instead.
--
-- Vaults:
...
-- ─── Parameters ─────────────────────────────────────────────────────────────
-- No adapter field: for V1 the vault itself is the direct Morpho Blue supplier.
params AS (
    SELECT
...
-- All Morpho Blue markets where the vault has ever supplied directly.
-- V1 difference from V2: onBehalf = vault (not a separate adapter address).
vault_markets AS (
    SELECT DISTINCT s.id AS market_id
    FROM morpho_blue_ethereum.morphoblue_evt_supply s
    CROSS JOIN params p
    WHERE s.onBehalf = p.vault
),

-- ─── Per-market running state: idle USDS and share ratios ───────────────────
-- V1 difference from V2: delta_vault_shares uses onBehalf = p.vault.
market_events AS (
    ...
    WHERE w.id IN (SELECT market_id FROM vault_markets)
    UNION ALL
    ...
    WHERE b.id IN (SELECT market_id FROM vault_markets)
    UNION ALL
    ...
    WHERE r.id IN (SELECT market_id FROM vault_markets)
),
...
market_state AS (
    SELECT block_number, log_index, market_id,
        ...
        SUM(delta_vault_shares) OVER w  AS vault_shares
    FROM market_events
    WINDOW w AS (PARTITION BY market_id ORDER BY block_number, log_index ROWS UNBOUNDED PRECEDING)
),

-- Vault's pro-rata share of unborrowed USDS in each market.
market_vault_idle AS (
    SELECT block_number, log_index, market_id,
        CASE WHEN total_shares > 0 AND vault_shares > 0
             THEN CAST(idle_usds AS DOUBLE) * CAST(vault_shares AS DOUBLE)
                  / CAST(total_shares AS DOUBLE)
             ELSE 0.0 END AS vault_idle
    FROM market_state
),

-- ─── Vault USDS cash balance (not yet deployed to markets) ──────────────────
-- V1 difference from V2: only tracks the vault address (no adapter contract).
-- No internal-transfer zero-out needed since all flows are external.
vault_usds_deltas AS (
    SELECT block_number, index AS log_index,
        CASE
            WHEN bytearray_substring(topic2, 13) = p.vault
    ...
    WHERE contract_address = p.usds_token
      ...
      AND (bytearray_substring(topic1, 13) = p.vault
           OR bytearray_substring(topic2, 13) = p.vault)
),
...
-- ─── Dynamic forward-fill: total vault idle across all discovered markets ───
-- Unified event timeline: every (block, log_index) from any source.
combined_events AS (
    SELECT block_number, log_index FROM market_vault_idle
    UNION
...
market_idle_expanded AS (
    SELECT
        ...
        e.log_index,
        vm.market_id,
        LAST_VALUE(mvi.vault_idle) IGNORE NULLS OVER (
            PARTITION BY vm.market_id
            ORDER BY e.block_number, e.log_index
            ROWS UNBOUNDED PRECEDING
        ) AS vault_idle
    FROM combined_events e
    CROSS JOIN vault_markets vm
    LEFT JOIN market_vault_idle mvi
        ON mvi.market_id   = vm.market_id
        AND mvi.block_number = e.block_number
        AND mvi.log_index    = e.log_index
),

-- Sum the forward-filled vault idle across all markets at each event point.
total_vault_idle_per_event AS (
    SELECT
        block_number,
        log_index,
        COALESCE(SUM(vault_idle), 0.0) AS total_vault_idle
...
idle_with_ts AS (
    SELECT
        ...
        (
            COALESCE(tvi.total_vault_idle, 0.0)
    ...
    LEFT JOIN total_vault_idle_per_event tvi
        ON tvi.block_number = vie.block_number
        AND tvi.log_index   = vie.log_index
