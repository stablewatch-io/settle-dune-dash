...
-- Query ID: 6864537
-- Description: Time-weighted USDS reward for the Curve stUSDS/USDS IB vault (vault level).
--              Tracks the pool's USDS (coins(0)) balance directly via ERC20 Transfer events.
--              No LP token balance or ratio conversion used.
--              No idle_factor applied — Curve capital is treated as fully productive.
--
-- Pool (fixed):
--   Pool / LP token: 0x2c7c98a3b1582d83c43987202aeff638312478ae  (Curve stUSDS/USDS)
--   coins(0) / USDS: 0xdc035d45d973e3ec169d2276ddab16f1e407384f
--   coins(1):        0x99cd4ec3f88a45940936f469e4bb72a2a701eeb9  (stUSDS — not tracked)
--
-- Parameters:
...
--
-- NOTE: Calculation CTEs are intentionally duplicated from ib-vaults-rewards-aave.
--       Dune SQL has no UDF/macro support; shared raw data (query_6853959) is the
--       extent of practical abstraction possible.
...
-- ─── USDS (coins(0)) transfers to/from the pool ───────────────────────────────
-- Signed delta: positive when USDS flows into the pool, negative when it flows out.
usds_deltas AS (
    SELECT
        block_number,
        index                                                         AS log_index,
